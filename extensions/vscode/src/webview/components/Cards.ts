...
export const impactCard = (item: ImpactItem, labels: { confirmed: string; likely: string; possible: string; confidenceLow: string; reason: string }): string => {const possible=item.tier==="possible";const tier=possible?labels.possible:item.tier==="likely"?labels.likely:labels.confirmed;const location=`${item.file || ""}${item.line ? `:${item.line}` : ""} · ${possible?labels.confidenceLow:item.confidence}`;return `<button class="result-card impact-card impact-card--${esc(item.tier)}" data-entity="${esc(item.entityId)}" data-file="${esc(item.file)}" data-line="${item.line || ""}"><span class="result-card__kind">${esc(tier)} · ${esc(item.kind)}</span><strong>${esc(item.label)}</strong><span class="result-card__reason">${possible?`${esc(labels.reason)}: `:""}${esc(item.reason || item.kind)}</span><small title="${esc(location)}">${esc(location)}</small></button>`;};
...
export const testCard = (item: TestRecommendation, index: number, run: string, language: "ru" | "en" = "en"): string => {
  ...
  return `<article class="test-card"><div><span class="result-card__kind">${esc(category)}</span><strong title="${esc(item.file || item.symbol)}">${esc(item.file || item.symbol)}</strong><p>${esc(item.reason)}</p><small title="${esc(detail)}">${esc(detail)}</small></div>${item.argv ? `<button class="button button--secondary" data-test="${index}">${esc(run)}</button>` : ""}</article>`;
};
