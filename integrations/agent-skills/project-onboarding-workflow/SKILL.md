...
Use the Graphify artifact only for broad architecture exploration. Use the canonical CodeSlicer graph for impact, PR risk, evidence and targeted tests. Before a commit run `impact-engine --json review <project> --run-tests suggested`; it only selects tests. Execute a suggested test only after a developer confirms the command.
