...
Кнопка **«Практикум»** открывает отдельную страницу обучения. В ней можно выбрать один из четырёх сценариев: проверить текущие изменения, сравнить с веткой, разобраться в архитектуре или проверить GitHub PR. У каждого сценария есть понятные шаги, индикатор прогресса и кнопка **«Показать в интерфейсе»**, которая только переключает вкладку и подсвечивает нужный элемент. Настройка, OAuth, review и открытие локального обзора запускаются только отдельной явно подписанной кнопкой **«Выполнить…»**.
