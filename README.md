...
В расширении VS Code тот же путь доступен с первого экрана: выберите
**«Установить за пару кликов»** → **«Установить CodeSlicer»**. Одно нажатие
скачает официальный архив в служебную папку VS Code, создаст `.venv` и сразу
подключит локальный `codeslicer.exe`. Не нужно выбирать папку или указывать
путь. Необязательный второй шаг **«Подключить IDE и skills»** открывает
существующее интерактивное меню IDE.
...
При первом открытии CodeSlicer сразу показывает **«Практикум»**. Выберите один из четырёх сценариев: проверить текущие изменения, сравнить с веткой, разобраться в архитектуре или проверить GitHub PR. Первый и каждый следующий шаг автоматически переключают нужную вкладку и подсвечивают точный элемент; компактная карточка сопровождения остаётся поверх интерфейса. Настройка, OAuth, review и открытие локального обзора запускаются только отдельной явно подписанной кнопкой **«Выполнить…»**.
...
В первом шаге есть кнопка **«Установить CodeSlicer»**. Одно нажатие скачивает
официальный ZIP, распаковывает его в служебную папку VS Code, создаёт `.venv`
и автоматически подключает executable. Папка назначения и ручная настройка
пути не нужны.

- **IDE и skills — по желанию** — открывает существующее интерактивное меню
  PowerShell только после отдельного нажатия. Отметьте нужные IDE, затем
  нажмите Enter.
...
Установка начинается только по отдельной явно подписанной кнопке, не просит
токены и не меняет workspace.
