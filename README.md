...
**«Установить и настроить»** → **«Открыть помощник установки»**. После выбора
папки назначения расширение скачает официальный ZIP прямо в неё и распакует
`CodeSlicer-main`; затем покажет подтверждение перед PowerShell-установщиком.
Отдельный маршрут **«Подключить IDE и
skills»** запускает существующее интерактивное меню IDE из расширения после
настройки локального `codeslicer.exe`.
