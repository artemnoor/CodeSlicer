...
Кнопка **«Как это работает»** запускает встроенный пятишаговый тур: он сам переключает вкладки, подсвечивает нужный элемент и кратко объясняет следующий шаг. В туре можно вернуться назад, пропустить его или нажать `Esc`; он никогда не запускает review, тесты, загрузки или настройку runtime.
