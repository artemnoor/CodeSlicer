...
sourceCard = function simpleSourceCard(item) {
  ...
  if (id === 'lsp') { const row = el('div', 'adapter-actions'), input = el('input'); attr(input, { placeholder: 'Абсолютный путь к локальному LSP', 'data-lsp-executable': 'true' }); append(row, input, button('Настроить', 'button secondary', { 'data-adapter-action': 'configure-lsp', 'data-adapter-id': id }), button('Проверить', 'button secondary', { 'data-adapter-action': 'lsp-probe', 'data-adapter-id': id }), button('Отключить', 'button secondary', { 'data-adapter-action': 'lsp-disable', 'data-adapter-id': id })); card.append(el('p', 'muted', 'LSP — локальный процесс: укажите путь, затем проверьте соединение.'), row); }
...
async function handleAdapterAction(action) { const id = action.dataset.adapterId; const project_path = state.project; const original = action.textContent; action.disabled = true; try { let response; if (action.dataset.adapterAction === 'import') { const path = $(`[data-adapter-path="${CSS.escape(id)}"]`)?.value.trim(); if (!path) throw new Error('Укажите абсолютный путь к локальному артефакту.'); response = await ImpactApi.adapterImport(id, { project_path, artifact_path: path }); } else if (action.dataset.adapterAction === 'enable') response = await ImpactApi.adapterEnable(id, { project_path }); else if (action.dataset.adapterAction === 'configure-native') { const executable = $(`[data-native-executable="${CSS.escape(id)}"]`)?.value.trim(); if (!executable) throw new Error('Укажите абсолютный путь к уже установленному локальному executable.'); response = await ImpactApi.nativeConfigure(id, { project_path, executable }); } else if (action.dataset.adapterAction === 'configure-lsp') { const executable = $('[data-lsp-executable]')?.value.trim(); if (!executable) throw new Error('Укажите абсолютный путь к уже установленному локально LSP.'); response = await ImpactApi.lspConfigure({ project_path, executable, workspace_roots: [project_path] }); } else if (action.dataset.adapterAction === 'lsp-probe') response = await ImpactApi.lspProbe({ project_path }); else if (action.dataset.adapterAction === 'lsp-disable') response = await ImpactApi.lspDisable({ project_path }); else if (action.dataset.adapterAction === 'otel-live-enable') response = await ImpactApi.otelLiveEnable({ project_path }); else if (action.dataset.adapterAction === 'otel-live-disable') response = await ImpactApi.otelLiveDisable({ project_path }); else if (action.dataset.adapterAction === 'disable') response = await ImpactApi.adapterDisable(id, { project_path }); if (response) showState(`${id}: действие выполнено локально.`); await loadSources(true); } catch (error) { showState(`${id}: ${error.message}`, 'error-state'); } finally { action.disabled = false; action.textContent = original; } }
