...
const riskLevelLabel = (risk) => ({ CRITICAL: 'КРИТИЧЕСКИЙ', HIGH: 'ВЫСОКИЙ', MEDIUM: 'СРЕДНИЙ', LOW: 'НИЗКИЙ', UNKNOWN: 'НЕ ОПРЕДЕЛЁН' }[riskLevel(risk)] || 'НЕ ОПРЕДЕЛЁН');
const confidenceLabel = (value) => {
  if (typeof value === 'number' && Number.isFinite(value)) return `${Math.round(value * 100)}%`;
  return ({ high: 'Высокая', medium: 'Средняя', low: 'Низкая', unknown: 'Не определена' }[String(value || '').toLowerCase()] || textOf(value, '—'));
};
...
function reviewWarningCopy(warning) {
  const raw = String(warning || '').trim();
  const suppressed = raw.match(/(\d+)\s+low-value entities suppressed/i);
  if (suppressed) return { title: 'Второстепенные сущности скрыты', detail: `${suppressed[1]} служебных, встроенных или сгенерированных сущностей не показаны, чтобы карта оставалась читаемой.` };
  if (/no cross-file impact proven|no_cross_file_impact_proven/i.test(raw)) return { title: 'Межфайловая цепочка не подтверждена', detail: 'Для текущих изменений не найдено достаточно доказательств связи между файлами.' };
  if (/no local changes were found/i.test(raw)) return { title: 'Изменения не найдены', detail: 'Выберите ветку сравнения или передайте diff, чтобы получить оценку влияния.' };
  return { title: 'Ограничение анализа', detail: raw || 'Backend не передал подробностей.' };
}
function renderWarnings(target, warnings) {
  clear(target);
  const items = [];
  const seen = new Set();
  unique(warnings).map(reviewWarningCopy).forEach((item) => {
    const key = `${item.title}\n${item.detail}`;
    if (!seen.has(key)) { seen.add(key); items.push(item); }
  });
  if (!items.length) return;
  const panel = el('section', 'analysis-limits');
  append(panel, cardTitle('ОГРАНИЧЕНИЯ АНАЛИЗА', 'Что важно учесть', `${items.length} ${items.length === 1 ? 'пояснение' : 'пояснения'}`));
  const list = el('ul', 'analysis-limits-list');
  items.forEach((item) => { const row = el('li'); append(row, el('strong', '', item.title), el('span', '', item.detail)); list.append(row); });
  panel.append(list); target.append(panel);
}
...
function renderReview() {
  ...
  const left = el('aside', 'panel file-list'); append(left, cardTitle('ИЗМЕНЕНИЯ', 'Изменённые файлы', files.length ? `${files.length} файлов` : 'Пустой diff')); const filters = el('div', 'filter-row'); [['all', 'Все'], ['api', 'API'], ['tests', 'Тесты'], ['high', 'Высокий риск']].forEach(([value, label]) => { const f = button(label, `filter-button ${state.reviewFilter === value ? 'active' : ''}`, { 'data-review-filter': value }); filters.append(f); }); left.append(filters); if (!files.length) left.append(stateCard('Изменений относительно выбранной ветки не найдено', 'Проверьте базовую ветку или передайте diff явно.', 'empty-state')); filteredFiles.forEach((file) => { const row = button('', `file-row ${state.selectedFile === file.path ? 'selected' : ''}`, { 'data-review-file': file.path, 'aria-label': file.path }); append(row, el('span', 'status-key', isHighRisk(review) ? '!' : '·'), el('span', '', '')); row.lastChild.append(el('code', '', file.path), el('small', 'muted', `${file.additions} добавлено · ${file.deletions} удалено · ${languageFor(file.path)}`)); left.append(row); }); layout.append(left);
  const center = el('div', 'review-center'); const risk = review.risk || {}; const riskCard = el('article', 'panel impact-summary'); const level = riskLevel(risk); append(riskCard, cardTitle('РИСК И ВЛИЯНИЕ', level === 'UNKNOWN' ? 'Риск не определён' : `${riskLevelLabel(risk)} · ${level === 'HIGH' || level === 'CRITICAL' ? 'Высокое влияние' : 'Ограниченное влияние'}`, `Оценка относится к текущему Review, не к запуску теста.`)); const banner = el('div', `risk-banner ${level.toLowerCase()}`); append(banner, el('span', 'status-key', level === 'HIGH' || level === 'CRITICAL' ? '!' : '?'), el('span', '', describe(risk.reason || risk.reasons || 'Backend не передал текст причины.'))); riskCard.append(banner); const stats = el('div', 'impact-stats'); append(stats, metric('Изменённые файлы', files.length), metric('Главные затронутые сущности', impacts.length), metric('Уверенность', confidenceLabel(risk.confidence))); riskCard.append(stats); center.append(riskCard);
  ...
  const evidence = el('article', 'panel evidence-card'); append(evidence, cardTitle('ДОКАЗАТЕЛЬСТВА', 'Подтверждённые цепочки', 'Ограничения анализа показаны выше отдельным списком.')); (review.chains || []).slice(0, 5).forEach((chain) => { const row = el('div', 'evidence-row'); append(row, el('span', 'status-key', chain.status === 'confirmed' ? '✓' : '?'), el('div', '', '')); row.lastChild.append(el('strong', '', `${confidenceLabel(chain.confidence)} · ${chain.summary || chain.id || 'Цепочка'}`), el('small', 'muted', `Статус: ${statusLabel(chain.status)} · Источники: ${unique(chain.evidence_ids || []).join(', ') || 'не указаны'}`)); evidence.append(row); }); if (!review.chains?.length) evidence.append(stateCard('Подтверждённых цепочек нет', 'Это не означает, что код исправен: для текущего diff недостаточно доказательств связи.', 'empty-state')); center.append(evidence); layout.append(center);
  const right = el('aside', 'panel test-panel'); append(right, cardTitle('ЛОКАЛЬНЫЕ ТЕСТЫ', 'Что проверить', 'Запуск только через /api/review/run-test после подтверждения.')); if (!tests.length) right.append(stateCard('Целевые тесты не предложены', review.incomplete ? 'Рекомендации скрыты из-за неполного покрытия.' : 'Резервный набор тестов не отдан backend.', 'empty-state')); tests.slice(0, 10).forEach((test) => right.append(testRow(test))); const runSelected = button('Запустить выбранные тесты', 'button primary', { 'data-action': 'run-selected-tests' }); runSelected.disabled = !tests.some((test) => test.file); right.append(runSelected); const fallback = review.fallback_suite || review.fallback_tests || review.review_projection?.fallback_suite; if (fallback) { right.append(el('h3', 'subheading', 'Резервный набор тестов')); right.append(el('p', 'muted', describe(fallback))); } const result = el('div', 'test-result'); if (state.lastTest) renderTestResult(result, state.lastTest); right.append(result); layout.append(right); target.append(layout);
}
...
function graphNodeLabel(node) { return textOf(node?.name || node?.label || node?.id, 'Безымянная сущность'); }
function graphNodeAccessibleLabel(node) { return truncate(graphNodeLabel(node).replace(/\s+/g, ' ').trim(), 120) || 'Безымянная сущность'; }
...
function createNetworkGraph(projection, { compact = false } = {}) {
  ...
  visibleNodes.forEach((node) => {
    ...
    const accessibleLabel = graphNodeAccessibleLabel(node);
    const isHub = node.id === ordered.hub?.id;
    const group = svgEl('g', { class: `projection-node-link network-node${isHub ? ' is-hub' : ''}${external ? ' is-external' : ''}`, tabindex: '0', role: 'button', 'aria-label': `${accessibleLabel}. ${isHub ? 'Центральный узел. ' : ''}Открыть сведения`, 'data-projection-entity': node.id });
    group.append(svgEl('title', {}, `${label} · ${node.kind || 'ENTITY'}`));
    ...
    group.append(svgEl('text', { x: point.x, y: point.y - (isHub ? (compact ? 18 : 24) : 11), class: `graph-label graph-node-caption${isHub ? ' is-visible' : ''}` }, truncate(label, compact ? 18 : 25)));
